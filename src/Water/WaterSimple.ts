...
class WaterSimple extends Mesh {
	constructor(geometry: any, options: WaterOptions = {}) {
		...
		const reflectivity =
			options.reflectivity !== undefined ? options.reflectivity : 1.0;
		const size =
			options.size !== undefined ? options.size : 10.0;
		...
		const mirrorShader = {
			...
			fragmentShader: /* glsl */ `
				...
				void main() {
					...
					// Fresnel: bords très réfléchissants, centre montre la couleur de l'eau
					float fresnel = pow( 1.0 - max( dot( eyeDirection, surfaceNormal ), 0.0 ), 3.0 );
					float reflectionAmount = mix( 0.02, reflectivity, fresnel );
					vec3 outgoingLight = mix( waterColor, reflectionSample, reflectionAmount );

					// Highlight spéculaire directionnel
					vec3 lightDir = normalize( vec3( 0.5, 1.0, 0.5 ) );
					float spec = pow( max( dot( reflect( -lightDir, surfaceNormal ), eyeDirection ), 0.0 ), 64.0 );
					outgoingLight += vec3( 0.8, 0.9, 1.0 ) * spec * 0.4;
